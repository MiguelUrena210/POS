...
let CouponsService = class CouponsService {
    ...
    async findOne(id) {
        ...
        if (!coupon)
            throw new common_1.NotFoundException('No se encontró un cupón con el id ' + id);
        return coupon;
    }
    ...
    async applyCoupon(name) {
        ...
        if (!coupon) {
            throw new common_1.NotFoundException('No se encontró un cupón con ese nombre');
        }
        const currentDate = new Date();
        const expirationDate = (0, date_fns_1.endOfDay)(coupon.expirationDate);
        if ((0, date_fns_1.isAfter)(currentDate, expirationDate)) {
            throw new common_1.UnprocessableEntityException('El cupón ya ha expirado');
        }
        return {
